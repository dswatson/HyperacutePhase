...
moderates <- paste0('Moderate_vs_Ctrl_', c('0h', '24h', '72h'))
colnames(des)[grepl('Moderate', colnames(des))] <- moderates
mods <- paste0(rep(c('noMODS_vs_Ctrl_', 'MODS_vs_Ctrl_'), each = 3), 
               c('0h','24h', '72h'))
colnames(des)[grepl('MODS', colnames(des))] <- mods

# Fit model
icc <- duplicateCorrelation(eset, des, block = clin$Subject) 
fit <- lmFit(eset, des, correlation = icc$cor, block = clin$Subject)  
cm <- makeContrasts('(MODS_vs_Ctrl)_vs_(noMODS_vs_Ctrl)_0h' = 
                      MODS_vs_Ctrl_0h - noMODS_vs_Ctrl_0h,
                    '(MODS_vs_Ctrl)_vs_(noMODS_vs_Ctrl)_24h' = 
                      MODS_vs_Ctrl_24h - noMODS_vs_Ctrl_24h,
                    '(MODS_vs_Ctrl)_vs_(noMODS_vs_Ctrl)_72h' = 
                      MODS_vs_Ctrl_72h - noMODS_vs_Ctrl_72h,
                    levels = des)
coefs <- colnames(cm)
...
# Refit model
icc <- duplicateCorrelation(eset, des, block = clin$Subject) 
urFit <- lmFit(eset, des, correlation = icc$cor, block = clin$Subject)  
fit <- eBayes(contrasts.fit(urFit, cm))

# Results 
res <- function(coef) {

  resid_mat <- residuals(urFit, eset)
